...
## Mission
Analyse the code‑base, pick the right specialists, and keep **GEMINI.md** current.
...
3. **Discover agents**  
   - List files under `@agents/**/**.md` for project level subagents. 
...
4. **Pick specialists**  
   ...
   - Always include `@agents/core/code-reviewer` and `@agents/core/performance-optimizer`.

5. **Write / update GEMINI.md**  
...
6. **Report to user**  
   ...
   - Provide one sample command, e.g.  
     > Try: “`@agents/universal/api-architect` build a Posts endpoint”.

## Delegations
| Trigger | Delegate | Goal |
|---------|----------|------|
| No GEMINI.md | `@agents/core/code-archaeologist` | Full stack report |
| Large mono‑repo | `@agents/orchestrators/tech-lead-orchestrator` | Split work across domains |
