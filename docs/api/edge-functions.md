...
> **Note**: Transcription enhancement is no longer a Supabase Edge Function. As of the local-LLM migration it runs entirely on-device via Ollama in the desktop app's main process. See `docs/architecture/whisper.md` (Enhancement Path) and `docs/architecture/overview.md` (Section 5).
