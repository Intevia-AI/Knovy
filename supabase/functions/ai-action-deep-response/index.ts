...
const handleRequest = async (req: Request, profile: Record<string, any>) => {
  ...
  try {
    console.log(`[ai-action-deep-response] function invoked at: ${new Date().toISOString()}`);
    ...
    const prompt = PROMPTS.deepResponse[lang].base({
      text_input,
      existing_summary,
    ...
    try {
      ...
      if (logError) {
        console.error("[ai-action-deep-response] Failed to log feature usage:", logError);
        // Don't fail the request if logging fails
      }
    } catch (logException) {
      console.error(
        "[ai-action-deep-response] Exception while logging feature usage:",
        logException,
      );
      // Don't fail the request if logging fails
    }
    ...
    console.error(`[ai-action-deep-response] Error: ${error.message}`);
    ...
    try {
      ...
      console.error("[ai-action-deep-response] Failed to log error:", logException);
    }
...
const deepResponseHandler = withEntitlements(
  "allow_ai_action:deep-response",
  "daily_ai_action:deep-response_calls",
  handleRequest,
);

if (import.meta.main) {
  serve(deepResponseHandler);
}
