...
Deno.test("ai-action-summarize handler", async (t) => {
  let envStub: any;
  let fetchStub: any;
  ...
  await t.step("OPTIONS preflight request", async () => {
    const req = new Request("http://localhost/ai-action-summarize", { method: "OPTIONS" });
    const response = await handler(req);
    assertEquals(response.status, 200);
    assertEquals(response.headers.get("Access-Control-Allow-Origin"), "*");
  });

  await t.step("Successful summarization", async () => {
    setup("test_api_key", {
      ok: true,
      json: () =>
        Promise.resolve({
    ...
    const req = new Request("http://localhost/ai-action-summarize", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ text: "This is a long text to summarize." }),
    });
    ...
    assertEquals(response.status, 200);
    assertEquals(body.summary, "This is a summary.");
