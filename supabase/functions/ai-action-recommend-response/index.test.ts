...
Deno.test("ai-action-recommend-response handler", async (t) => {
  let envStub: any;
  let fetchStub: any;
  ...
  await t.step("OPTIONS preflight request", async () => {
    const req = new Request("http://localhost/ai-action-recommend-response", { method: "OPTIONS" });
  ...
  await t.step("Successful response recommendation", async () => {
    ...
    const req = new Request("http://localhost/ai-action-recommend-response", {
      method: "POST",
