...
node -e "console.log(process.env.GOOGLE_GENERATIVE_AI_API_KEY ? 'API key set' : 'API key missing')"
```
...
grep -i "memory\|heap" logs/proxy.log
```
...
- **High Latency**: Check network connectivity to Google APIs
- **Connection Drops**: Verify rate limiting settings and client behavior
- **Memory Leaks**: Monitor connection cleanup and garbage collection
- **CPU Spikes**: Profile audio processing and WebSocket message handling
