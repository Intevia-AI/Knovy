    ...
    if (input.length > 0) {
      ...
      if (!this.isSilent) {
        ...
        if (!this.isInSpeech) {
          ...
          console.log('[MicAudioProcessor] Speech started')
        }
        ...
        if (this.isInSpeech && this.lastSpeechTime) {
          ...
          if (silenceDuration >= this.speechTimeout) {
            ...
            if (segmentDuration >= this.minSegmentLength) {
              console.log(`[MicAudioProcessor] Speech ended after ${segmentDuration}ms, triggering segment dispatch`)
      ...
      if (this.segmentStartTime) {
        ...
        if (segmentDuration >= this.maxSegmentLength) {
          console.log(`[MicAudioProcessor] Maximum segment duration (${segmentDuration}ms) reached, forcing dispatch`)
          this.port.postMessage({
            type: 'speechEnd',
            segmentDuration,
            forced: true,
